...
interface CommitteesClientProps {
  initialCommittees: CommitteeWithBills[];
}
...
export function CommitteesClient({ initialCommittees }: CommitteesClientProps) {
  ...
  const filteredCommittees = useMemo(() => {
    let committees = [...initialCommittees];
    ...
    return committees;
  }, [initialCommittees, chamberFilter, typeFilter, showOnlyWithHearings]);

  const senateCount = initialCommittees.filter((c) => c.committee.chamber === 'senate').length;
  const houseCount = initialCommittees.filter((c) => c.committee.chamber === 'house').length;
  const withHearingsCount = initialCommittees.filter((c) => c.upcomingHearings.length > 0).length;
