...
import type { Committee, Bill, BillCard as BillCardType, Hearing, Member } from '@/types';
...
interface CommitteeMemberWithRole extends Member {
  role: 'chair' | 'vice_chair' | 'ranking_member' | 'member';
}

interface CommitteeDetailClientProps {
  committee: Committee;
  bills: BillCardType[];
  members: CommitteeMemberWithRole[];
  upcomingHearings: Hearing[];
  hearingBillsMap: Record<string, Bill[]>;
}
            ...
            <section className="bg-mo-gold/10 rounded-xl border border-mo-gold/30 p-6">
              ...
              <div className="space-y-3">
                {upcomingHearings.map((hearing) => {
                  const hearingBills = hearingBillsMap[hearing.id] || [];
                  return (
                    <div
                      key={hearing.id}
